...
-->
  <div id="main">
    ...
    <h1><center>สินค้าที่ได้รับความนิยมสูงสุดในแต่ละหมวดสินค้า</center></h1><hr>
    <div class="container justify-content-md-center shadow-none p-3 mb-5 rounded" style="border:1px solid #cecece; width:80%" >
        ...
        <table class="table table-hover" style="width:100%">
                ...
                <tr>
                    ...
                    <th><h2><center>จำนวนที่ขายได้</center></h2></th>
                </tr>
                </thead>
                <tbody>
                <?php 
                $SQL = "SELECT z.ProductName ,z.CategoryType,MAX(z.Frequency) AS Frequency FROM (SELECT stock.ProductName,category.CategoryType,COUNT(stock.CategoryID) AS Frequency FROM checkout JOIN ordershop ON checkout.OrderDate BETWEEN '".$month."-1' AND '".$month."-31'  AND checkout.OrderID = ordershop.OrderID JOIN orderdetail ON ordershop.OrderDetailID = orderdetail.OrderDetailID JOIN productdetail ON orderdetail.ProductDetailID = productdetail.ProductDetailID JOIN stock ON productdetail.ProductID = stock.ProductID JOIN category ON stock.CategoryID = category.CategoryID GROUP BY category.CategoryType,stock.ProductName ORDER BY Frequency DESC) z GROUP BY z.CategoryType";
...
function closeNav() {
  document.getElementById("mySidepanel").style.width = "0";
    ...
    function changemonth(){
        ...
        window.location="analysis11.php?month="+month;
        console.log(month);
    }
  </script>
</body>
