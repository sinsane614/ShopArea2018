...
-->
  <div id="main">
    ...
    <h1><center>ประเภทสินค้าที่ผู้หญิงเลือกซื้อมากที่สุดในรอบเดือน</center></h1><hr>
    <div class="container justify-content-md-center shadow-none p-3 mb-5 rounded" style="border:1px solid #cecece; width:80%" >
        ...
        <table class="table table-hover" style="width:100%">
                ...
                <tr>
                    <th></th>
                    <th><h2><center>ประเภทสินค้า</center></h2></th>
                    <th><h2><center>จำนวนครั้งการซื้อ</center></h2></th>
                </tr>
                </thead>
                <tbody>
                <?php 
                $SQL = "SELECT category.CategoryType,COUNT(stock.CategoryID) AS Frequency FROM checkout JOIN ordershop ON checkout.OrderID = ordershop.OrderID JOIN orderdetail ON ordershop.OrderDetailID = orderdetail.OrderDetailID JOIN productdetail ON orderdetail.ProductDetailID = productdetail.ProductDetailID JOIN stock ON productdetail.ProductID = stock.ProductID  JOIN category ON stock.CategoryID = category.CategoryID JOIN user ON stock.SellerID = user.UserID WHERE checkout.OrderDate BETWEEN '".$month."-1' AND '".$month."-31'  AND user.Gender LIKE 'F' GROUP BY category.CategoryType ORDER BY Frequency DESC";
                ...
                while($row1 = $query->fetch_assoc()){
                    ...
                    <tr>
                        <td></td>
                        <td><h3><center><?php echo $row1['CategoryType'] ?></center></h3></td>
                        <td><h3><center><?php echo $row1['Frequency'] ?></center></h3></td>
                    </tr>
...
function closeNav() {
  document.getElementById("mySidepanel").style.width = "0";
    ...
    function changemonth(){
        ...
        window.location="analysis14.php?month="+month;
        console.log(month);
    }
  </script>
</body>
