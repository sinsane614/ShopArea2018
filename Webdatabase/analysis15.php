...
-->
  <div id="main">
    ...
    <h1><center>ช่วงอายุของลูกค้า ที่ซื้อสินค้าในรอบเดือน</center></h1><hr>
    <div class="container justify-content-md-center shadow-none p-3 mb-5 rounded" style="border:1px solid #cecece; width:80%" >
        ...
        <table class="table table-hover" style="width:100%">
                ...
                <tr>
                    <th></th>
                    <th><h2><center>ช่วงอายุ</center></h2></th>
                    <th><h2><center>จำนวนครั้งการซื้อ</center></h2></th>
                </tr>
                </thead>
                <tbody>
                <?php 
                $SQL = "SELECT t.blah AS age_range, count(*) AS COUNT FROM ( select case when age between 0 and 9 then ' 0-9 ' when age between 10 and 19 then '10-19' when age between 20 and 29 then '20-29' when age between 30 and 39 then '30-39' when age between 40 and 49 then '40-49' when age between 50 and 59 then '50-59' when age between 60 and 69 then '60-69' when age between 70 and 79 then '70-79' when age between 80 and 89 then '80-89' else '90-99' end as blah from
                                                                    
                                                                    
                                                                    (SELECT YEAR(CURRENT_TIMESTAMP) - YEAR(DOB) - (RIGHT(CURRENT_TIMESTAMP, 5) < RIGHT(DOB, 5)) as age FROM user u JOIN checkout c ON u.UserID=c.UserID AND c.OrderDate  BETWEEN '".$month."-1' AND '".$month."-31' ) s) t group by t.blah";
                ...
                while($row1 = $query->fetch_assoc()){
                    ...
                    <tr>
                        <td></td>
                        <td><h3><center><?php echo $row1['age_range'] ?></center></h3></td>
...
function closeNav() {
  document.getElementById("mySidepanel").style.width = "0";
    ...
    function changemonth(){
        ...
        window.location="analysis15.php?month="+month;
        console.log(month);
    }
  </script>
</body>
